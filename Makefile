...
ifeq ($(wildcard $(RTOOL)),)
  ...
  RTOOL_BUILD := $(shell $(HOSTCC) -std=c99 -O2 -D_POSIX_C_SOURCE=200809L -DZ7_ST -DZ7_NO_ASM -I$(RTOOL_SRC_DIR) -o $(RTOOL) $(RTOOL_SOURCES) -lm)
  ifeq ($(wildcard $(RTOOL)),)
    $(error Could not build $(RTOOL) from $(RTOOL_SRC_DIR); see the compiler errors above)
  endif
endif
...
NODE_SRC_DIR        := $(EMBEDDED_DIR)/export/resources/node_sources
...
EXPORT_RESOURCES_DIR   := $(EXPORT_DIR)/resources
NODE_SOURCES_ARCHIVE   := $(EXPORT_RESOURCES_DIR)/node_sources.tar.xz
...
EMBEDDED_FILES := $(sort $(EMBEDDED_FILES) $(ICON_ATLAS_PNG))
endif
...
$(NODE_SOURCES_ARCHIVE): $(NODE_SOURCES_ALL_FILES)
	$(info ==> Compressing src/nodes ($(words $(NODE_SOURCES_ALL_FILES)) files) into $(NODE_SOURCES_ARCHIVE))
		@$(RTOOL) pack $(NODE_SOURCES_ARCHIVE) $(EXPORT_RESOURCES_DIR) $(notdir $(NODE_SRC_DIR))
endif
